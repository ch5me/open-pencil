...
const { status, hostedAgent = false } = defineProps<{
...
function handleSubmit(e: Event) {
  ...
  if (!text || (hostedAgent && !hostedAgentOptions.ready.value)) return
  emit('submit', text)
  input.value = ''
}
</script>
      ...
      <form class="flex gap-1.5" @submit="handleSubmit">
        ...
        <Tip v-else :label="dialogs.sendMessage">
          <button
            ...
            :disabled="!input.trim() || (hostedAgent && !hostedAgentOptions.ready.value)"
          >
