...
import {
  ...
  createAgentServiceChatTransport
...
export function createChatSessionManager({
  ...
  async function ensureChat(): Promise<Chat<UIMessage> | null> {
    ...
    if (!chat || transportDirty || currentChatStore !== store) {
      ...
      let restoredHostedRun = false
      if (useHostedAgent) {
        const documentId = getActiveTabId()
        const hostedTransport = createAgentServiceChatTransport({
          store,
          documentId,
          clientId: documentId,
        ...
        restoredHostedRun =
          hostedTransport instanceof AgentServiceChatTransport && hostedTransport.hasRestoredRun()
      ...
      if (restoredHostedRun) void chat.resumeStream()
