...
| Hosted chat transport selection         | Hosted OpenPencil must use Firefly runtime and billing rather than upstream BYOK or local ACP         | Upstream supports an equivalent authority-selected remote transport                 | Select Firefly before provider/ACP choice in hosted mode; never fallback; require runtime ID and billing trace regressions                        |
