...
const h0 = JSON.parse(readFileSync(h0Path, 'utf8')) as {
  ...
  package: { name: string; version: string; exports: Record<string, unknown>; exportMapSha256: string }
...
const packageMetadata: Record<string, { dir: string; packageJson: PackageJson; tarball: string; tarballSha256: string }> = {}

for (const packageDir of packageDirs) {
  const packageJson = readPackage(`packages/${packageDir}`)
  const destination = join(tarballDir, packageTarballName(packageJson))
  run('bun', ['pm', 'pack', '--ignore-scripts', '--destination', tarballDir], join(root, `packages/${packageDir}`))
  assert(existsSync(destination), `missing packed tarball: ${destination}`)
  const packedPackageJson = JSON.parse(tarRead(destination, 'package.json').toString('utf8')) as PackageJson
  assert(packedPackageJson.name === packageJson.name, `packed name mismatch: ${packageJson.name}`)
  assert(packedPackageJson.version === packageJson.version, `packed version mismatch: ${packageJson.name}`)
  const packedManifest = JSON.stringify(packedPackageJson)
  assert(!packedManifest.includes('workspace:'), `packed manifest contains workspace dependency: ${packageJson.name}`)
...
const internalPackageMetadata: Record<
  string,
  { version: string; tarball: string; tarballSha256: string; integrity: string; publish: 'published' | 'already-present' }
> = {}
...
for (const packageName of packageNames) {
  ...
  run('npm', ['pack', '--ignore-scripts', '--pack-destination', internalTarballDir], packageRoot, registryEnv)
...
for (const exportName of requiredSceneGraphExports) {
  const exportEntry = packedSceneGraph.exports?.[exportName] as { types?: string; import?: string } | undefined
...
assert(
  sha256(tarRead(sceneGraphTarball, 'dist/id-allocation.d.ts')) === h0.declarations.idAllocation.sha256,
...
for (const exportName of requiredCoreExports) {
  const exportEntry = packedCore.exports?.[exportName] as { types?: string; import?: string } | undefined
...
const runRuntime = (consumerRoot: string, runtime: 'bun' | 'node') => {
  ...
  `
  run(runtime, runtime === 'bun' ? ['-e', source] : ['--input-type=module', '-e', source], consumerRoot)
}
...
writeFileSync(join(internalConsumerApp, 'package.json'), `${JSON.stringify(internalAppPackage, null, 2)}\n`)
writeFileSync(
...
assert(!internalLockText.includes('workspace:'), 'internal consumer lockfile contains workspace dependency')
...
const declarations = Object.fromEntries(
  ['root', 'variables', 'history', 'idAllocation'].map((key) => {
    ...
    assert(sha256(readFileSync(path)) === declaration.sha256, `declaration hash drift: ${declaration.path}`)
