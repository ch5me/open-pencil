...
import { deriveOperatingMode, validateHostedConfig, type HostedEnvironmentConfig } from '../packages/core/src/hosted/types.ts'
...
assert(topology.environments.staging.apiOrigin === 'https://api.staging.pencil.ch5.me',
  'staging apiOrigin matches envs.yaml API URL')
assert(topology.environments.production.apiOrigin === 'https://api.pencil.ch5.me',
...
assert(collabErrs.some(e => e.includes('hostedAuth')), 'violation mentions hostedAuth requirement')
...
assert(apiErrs.some(e => e.includes('apiOrigin')), 'hosted without apiOrigin produces violation')
...
assert(cbErrs.some(e => e.includes('authCallbackUrl')), 'hostedAuth without callback URL produces violation')

// --- 4. Operating mode derivation ---
console.log('\n4. Operating mode derivation')
assert(deriveOperatingMode({ hostedAuth: false, hostedDocs: false, hostedCollab: false }) === 'local-only',
  'all off -> local-only')
assert(deriveOperatingMode({ hostedAuth: true, hostedDocs: false, hostedCollab: false }) === 'hosted-auth-local-docs',
  'auth only -> hosted-auth-local-docs')
assert(deriveOperatingMode({ hostedAuth: true, hostedDocs: true, hostedCollab: false }) === 'hosted-docs-single-user',
  'auth+docs -> hosted-docs-single-user')
assert(deriveOperatingMode({ hostedAuth: true, hostedDocs: true, hostedCollab: true }) === 'hosted-collab',
...
assert(localFlags.hostedAuth === false && localFlags.hostedDocs === false && localFlags.hostedCollab === false,
  'local: all flags off')
assert(previewFlags.hostedAuth === true && previewFlags.hostedDocs === false && previewFlags.hostedCollab === false,
  'preview: auth only')
assert(stagingFlags.hostedAuth === true && stagingFlags.hostedDocs === true && stagingFlags.hostedCollab === false,
  'staging: auth+docs')
assert(prodFlags.hostedAuth === true && prodFlags.hostedDocs === true && prodFlags.hostedCollab === false,
...
assert(JSON.stringify(previewFlags) !== JSON.stringify(stagingFlags), 'preview ≠ staging (independent)')
...
assert(topology.environments.staging.authCallbackUrl !== topology.environments.production.authCallbackUrl,
