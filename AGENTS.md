...
- Hosted agent chat is provider-neutral and gated by `hostedAgent`, independently of hosted auth, docs, and collaboration. ELF authenticates product admission only; the externally operated generic gateway owns agent loops, routing, models, providers, billing, workers, and its deployment infrastructure. OpenPencil neither deploys that service nor claims that `.ch5/environments.yaml` does. Configure the OpenPencil API with `OPENPENCIL_AGENT_GATEWAY_ORIGIN`; project the API-only `OPENPENCIL_AGENT_GATEWAY_TOKEN` through Hush and never expose it in a `VITE_*` variable or browser code. OpenPencil owns chat UX, fail-closed approval, run cancellation, current-run reconnect, tool continuation, and guarded execution of canonical `ToolDef` actions. Select the hosted transport before BYOK/ACP and never fall back. The app/API contract must not expose runtime, container, image, registry, model, provider, or billing fields. Keep local contract execution deterministic and maintain exact upstream path/symbol rows in `docs/ch5/upstream-drift.md`.
