...
- Hosted agent chat is provider-neutral. OpenPencil owns ELF session handling, chat UX, and execution of its existing design `ToolDef` actions; the remote gateway owns agent loops, routing, models, providers, billing, workers, and deployment infrastructure. The app and API must not provision runtimes or expose container, image, registry, model, provider, or billing concepts. Keep the durable boundary and migration plan in `docs/ch5/agent-chat-boundary.md`.
