...
export async function readCh5ReviewContext(path: string): Promise<Ch5ReviewContext> {
  ...
  if (context.schema !== CONTEXT_SCHEMA)
    throw new Error(`context.schema must be ${CONTEXT_SCHEMA}`);
  ...
  const targetCensus = context.targetCensus
    ...
    .sort(
      (a, b) =>
        a.producerRuleId.localeCompare(b.producerRuleId) || a.nodeId.localeCompare(b.nodeId),
    );
  ...
  if (context.revision !== undefined)
    result.revision = string(context.revision, "context.revision");
  return result;
}
...
export async function readStableDocument(
  ...
  if (suppliedPath !== expectedPath)
    throw new Error("lint file must match context.documentLocator");
