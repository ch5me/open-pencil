...
async function measure(
  ...
  const directories = [...measuredDirectories.values()].sort((a, b) => a.path.localeCompare(b.path))
...
export async function captureImplementationProvenance(
  ...
  if (sourceRoot || import.meta.url.endsWith('.ts')) {
    ...
    return measure('source', cliRoot, await runtimeTopology(cliRoot, localPackages), repositoryRoot)
  }
...
export async function verifyImplementationProvenance(
  ...
  if (!sameTopology(before, topology)) {
    throw new Error('implementation provenance dependency resolution changed during lint execution')
  }
