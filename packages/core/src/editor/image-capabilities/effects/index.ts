...
function seededNoise(pixel: EffectPixel): number {
  let value = (pixel[0] * 374761393 + pixel[1] * 668265263 + pixel[2] * 2147483647 + pixel[3]) | 0
  ...
  switch (kind) {
    case 'levels': {
      ...
      return ([r, g, b, a]) => {
        const map = (value: number) =>
          outputBlack +
          ((Math.max(inputBlack, Math.min(inputWhite, value)) - inputBlack) /
    ...
    case 'vibrance': {
      ...
      return ([r, g, b, a]) => {
        ...
        return [
          clampByte(average + (r - average) * factor),
          clampByte(average + (g - average) * factor),
          clampByte(average + (b - average) * factor),
          a
        ]
      }
    }
    ...
    case 'gradient-map': {
      const start: [number, number, number] = [
        control(adjustments, 'startR', 0),
        control(adjustments, 'startG', 0),
        control(adjustments, 'startB', 0)
      ]
      const end: [number, number, number] = [
        control(adjustments, 'endR', 255),
        control(adjustments, 'endG', 255),
        control(adjustments, 'endB', 255)
      ]
      return ([r, g, b, a]) => {
        const amount = (r * 0.299 + g * 0.587 + b * 0.114) / 255
        return [
          clampByte(start[0] + (end[0] - start[0]) * amount),
          clampByte(start[1] + (end[1] - start[1]) * amount),
          clampByte(start[2] + (end[2] - start[2]) * amount),
          a
        ]
      }
    }
    ...
    case 'shadows-highlights': {
      ...
      return ([r, g, b, a]) => {
        ...
        const amount =
          luminance < 0.5 ? shadows * (1 - luminance * 2) : highlights * ((luminance - 0.5) * 2)
        return [
          clampByte(r + amount * 255),
          clampByte(g + amount * 255),
          clampByte(b + amount * 255),
          a
        ]
      }
    }
...
export function assertEffectPixelAcceptance(
  ...
  for (let row = 0; row < height; row += 1) {
    for (let column = 0; column < width; column += 1) {
      const inside = column >= x && column < x + areaWidth && row >= y && row < y + areaHeight
