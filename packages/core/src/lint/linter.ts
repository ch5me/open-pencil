...
export class Linter {
  ...
  lintChecks(graph: SceneGraph, checks: Array<{ ruleId: string; nodeId: string }>): LintResult {
    ...
    for (const { ruleId, nodeId } of checks) {
      ...
      if (!node) throw new Error(`lint target node not found: ${nodeId}`)
      const rule = this.rules.get(ruleId)
      if (!rule) throw new Error(`lint target rule is not enabled: ${ruleId}`)
      this.lintRule(ruleId, rule, node)
    }
    return this.result()
  }
  ...
  private lintRule(ruleId: string, rule: Rule, node: LintNode) {
    if (rule.match && !rule.match.includes(node.type)) return
    const config = this.ruleConfigs.get(ruleId)
    if (!config || config.severity === 'off') return
