...
test('image render adapter tracks revisions independently and reports zero unchanged uploads', () => {
  ...
  const revisions = new Map<string, AssetRevision>([
    ...
    [
      'sha256:hero-2',
      { revisionId: 'sha256:hero-2', kind: 'image', metadata: {}, bytes: new Uint8Array([2]) }
    ],
    [
      'sha256:mask-1',
      { revisionId: 'sha256:mask-1', kind: 'image', metadata: {}, bytes: new Uint8Array([3]) }
    ]
  ])
  const bindings = new Map([
    ['asset:hero', 'sha256:hero-1'],
...
test('image render adapter exposes typed gaps for missing and mismatched bindings', () => {
  const missing = createImageRenderAdapter().render(
    imagePlan([imageNode('missing', 'asset:missing')]),
    {
      getAsset: (assetId) =>
  ...
  const mismatch = createImageRenderAdapter().render(
    imagePlan([imageNode('mismatch', 'asset:mismatch')]),
    {
