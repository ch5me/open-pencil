...
describe('image tile planning', () => {
  test('plans only tiles intersecting a dirty rectangle', () => {
    ...
    expect(plan.tiles).toEqual([{ column: 1, row: 0, x: 256, y: 0, width: 256, height: 256 }])
  })
  ...
  test('does not include the next tile when dirty bounds end on a tile edge', () => {
    ...
    expect(plan.tiles).toEqual([{ column: 0, row: 0, x: 0, y: 0, width: 256, height: 256 }])
  })
  ...
  test('scales dirty rectangles into proxy tile coordinates', () => {
    ...
    expect(plan.tiles).toEqual([{ column: 1, row: 0, x: 128, y: 0, width: 128, height: 128 }])
  })
  ...
  test('rejects invalid dimensions and unbounded tile plans', () => {
    ...
    expect(() => createImageTilePlan({ sourceWidth: 1, sourceHeight: 1, tileSize: 1.5 })).toThrow(
      'invalid tile size'
    )
    expect(() =>
      createImageTilePlan({
