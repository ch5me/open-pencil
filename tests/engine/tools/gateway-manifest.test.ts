...
import { createGatewayManifest, defineTool, GATEWAY_REMOTE_POLICIES, toolToGatewayAction } from '@open-pencil/core/tools'

describe('gateway ToolDef manifest', () => {
  const resize = defineTool({
    name: 'node_resize', description: 'Resize', mutates: true,
    params: {
  ...
  const privateTool = defineTool({ name: 'private_action', description: 'Private', params: {}, execute: () => null })
  ...
  test('binds manifest identity to canonical content', async () => {
    ...
    expect((await createGatewayManifest([resize])).manifestId).not.toBe((await createGatewayManifest([changed])).manifestId)
  })
})
